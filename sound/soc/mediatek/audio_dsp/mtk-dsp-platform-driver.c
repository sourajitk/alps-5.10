...
#define OPLUS_MTK_UNDERRUN_FEEDBACK_CNT   (3)
				       ...
				       struct ipi_msg_t *ipi_msg, int id)
{
	...
	if (!snd_pcm_running(dsp->dsp_mem[id].substream)) {
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_MM_FEEDBACK)
		if ((SNDRV_PCM_STATE_XRUN == dsp->dsp_mem[id].substream->runtime->status->state) && \
				ipi_msg && (TASK_SCENE_FAST != ipi_msg->task_scene)) {
			dsp->dsp_mem[id].underflow_cnt++;
		}
#endif /* CONFIG_OPLUS_FEATURE_MM_FEEDBACK */
		return;
