	...
	struct iris_cmd_set *cmdset)
{
	...
	if (!iris_is_pt_mode()) {
		IRIS_LOGI("[%s](%d) is not in pt mode.", __func__, __LINE__);
		ret = -2;
		return ret;
	}
	...
	if (cmdset->count == 1 && cmdset->cmds[0].msg.type == MIPI_DSI_DCS_READ) {
		do { 
			...
			IRIS_LOGD("[%s](%d) retry = %d.", __func__, __LINE__, retry);
		} while ((ret <= 0) && (--retry));
	}
	else {
...
int iris_panel_read_status(int i, unsigned char cmd)
{
	...
	if ((0 <= i) && (i < ESD_CHK_NUM))
	{
		...
		if (iris_is_pt_mode())
			...
			IRIS_LOGI("[%s][%d] is not in pt mode.", __func__, i);
			pcfg->esd_chk_val[i] = ESD_CHK_EXIT;
			ret = 1;
		}

		IRIS_LOGD("[%s]esd_chk_val: 0x%02x", __func__, pcfg->esd_chk_val[i]);
	}
