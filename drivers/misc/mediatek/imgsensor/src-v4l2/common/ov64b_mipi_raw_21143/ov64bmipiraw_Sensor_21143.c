...
static void read_EepromQSC(struct subdrv_ctx *ctx)
{
    ...
    for (i = 0; i < 720; i ++) {
        ...
        ov64b_QSC_OVPD_setting[2*i+1] = read_cmos_eeprom_8(ctx, (addr_ovpd+i));
    }
    /*Read crosstalk*/
    for (i = 0; i < 288; i ++) {
        ov64b_QSC_CT_setting[2*i] = senaddr_crotalk+i;
        ov64b_QSC_CT_setting[2*i+1] = read_cmos_eeprom_8(ctx, (addr_crotalk+i));
    }
}
...
{
    LOG_INF("%s start\n", __func__);
    table_write_cmos_sensor(ctx, ov64b_QSC_OVPD_setting, sizeof(ov64b_QSC_OVPD_setting) / sizeof(kal_uint16));
    LOG_INF("%s end\n", __func__);
}
...
static void write_sensor_CT_QSC(struct subdrv_ctx *ctx)
{
    ...
    table_write_cmos_sensor(ctx, ov64b_QSC_CT_setting, sizeof(ov64b_QSC_CT_setting) / sizeof(kal_uint16));
    }
    LOG_INF("%s end\n", __func__);
}
...
static void write_sensor_crossTalk_data(struct subdrv_ctx *ctx)
{
    ...
    if(!crossTalk_flag) { //crosstalk Flag valid
        table_write_cmos_sensor(ctx, addr_data_pair_crossTalk_ov64b2q, sizeof(addr_data_pair_crossTalk_ov64b2q) / sizeof(kal_uint16));
        crossTalk_flag = 1;
    }
    LOG_INF("%s end\n", __func__);
}
...
static void preview_setting(struct subdrv_ctx *ctx)
{
	...
	if (!_is_seamless) {
		...
		memcpy((void *) (_i2c_data + _size_to_write),
			addr_data_pair_preview_OV64B2q_21143,
			sizeof(addr_data_pair_preview_OV64B2q_21143));
		_size_to_write += _length;
	}
	LOG_INF("%s end\n", __func__);
}
...
//			data_xtalk_OV64B2q_21143, sizeof(data_xtalk_OV64B2q_21143));
//#else
//		table_write_cmos_sensor(ctx, addr_data_pair_xtalk_OV64B2q_21143,
//			sizeof(addr_data_pair_xtalk_OV64B2q_21143)/sizeof(kal_uint16));
//#endif
...
static void custom5_setting(struct subdrv_ctx *ctx)
{
	...
	if (!_is_seamless) {
		...
		memcpy((void *) (_i2c_data + _size_to_write),
			addr_data_pair_custom5_OV64B2q_21143,
			sizeof(addr_data_pair_custom5_OV64B2q_21143));
		_size_to_write += _length;
	}
...
{
	kal_uint16 idx = 0;
	for (idx =0 ; idx <OTP_SIZE ; idx++) {
		otp_data[idx]=read_cmos_eeprom_8(ctx,idx);
	}
}
...
{
	kal_uint16 idx = 0;
	for (idx =0 ; idx <CALI_DATA_MASTER_LENGTH ; idx++) {
		stereo_data[idx]=read_cmos_eeprom_8(ctx,stereo_start_add+idx);
	}
}
...
static kal_uint32 set_test_pattern_mode(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	if (enable) {
        write_cmos_sensor_8(ctx, 0x0304, 0x00);
        write_cmos_sensor_8(ctx, 0x0305, 0xba);
        write_cmos_sensor_8(ctx, 0x0324, 0x00);
        write_cmos_sensor_8(ctx, 0x0325, 0xf0);
        write_cmos_sensor_8(ctx, 0x034b, 0x00);
        write_cmos_sensor_8(ctx, 0x380c, 0x06);
        write_cmos_sensor_8(ctx, 0x380d, 0xc0);
        write_cmos_sensor_8(ctx, 0x4837, 0x11);
        write_cmos_sensor_8(ctx, 0x4850, 0x43);

        write_cmos_sensor_8(ctx, 0x5000, 0x81);
        write_cmos_sensor_8(ctx, 0x5001, 0x40);
        write_cmos_sensor_8(ctx, 0x5002, 0x92);
        write_cmos_sensor_8(ctx, 0x5081, 0x81);
	} else {
		if (ctx->test_pattern) {
        write_cmos_sensor_8(ctx, 0x0304, 0x01);
        write_cmos_sensor_8(ctx, 0x0305, 0x75);
        write_cmos_sensor_8(ctx, 0x0324, 0x01);
        write_cmos_sensor_8(ctx, 0x0325, 0xe0);
        write_cmos_sensor_8(ctx, 0x034b, 0x00);
        write_cmos_sensor_8(ctx, 0x380c, 0x06);
        write_cmos_sensor_8(ctx, 0x380d, 0xc0);
        write_cmos_sensor_8(ctx, 0x4837, 0x08);
        write_cmos_sensor_8(ctx, 0x4850, 0x43);

        write_cmos_sensor_8(ctx, 0x5000, 0xCB);
        write_cmos_sensor_8(ctx, 0x5001, 0x43);
        write_cmos_sensor_8(ctx, 0x5002, 0x9E);
        write_cmos_sensor_8(ctx, 0x5081, 0x80);
		}
	}
