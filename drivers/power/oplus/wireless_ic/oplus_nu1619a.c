...
static int nu1619_get_work_freq(struct oplus_chg_ic_dev *dev, int *val)
{
	...
	rc = nu1619_read_data(chip, NU1619_REG_FREQ, &temp, 1);
	if (rc < 0) {
...
static int nu1619_get_headroom(struct oplus_chg_ic_dev *dev, int *val)
{
	...
	rc = nu1619_read_byte(chip, NU1619_REG_HEADROOM_R, &temp);
	if (rc) {
...
static int nu1619_set_headroom(struct oplus_chg_ic_dev *dev, int val)
{
	...
	rc = nu1619_write_byte(chip, NU1619_REG_HEADROOM_W, val);
	if (rc)
...
/*static u8 nu1619_calculate_checksum(const u8 *data, int len)
{
	u8 temp = 0;
...
static int nu1619_set_fod_parm(struct oplus_chg_ic_dev *dev, u8 data[], int len)
{
	...
	rc = nu1619_write_data(chip, 0x0068, data, len);
	if (rc < 0)
...
static int nu1619_wirte_firmware_data(struct oplus_nu1619 *chip, unsigned short addr, unsigned short length)
{
	...
	if (fw_data == NULL) {
		pr_err("oplus_nu1619 cannot get fw data\n");
		return -ENODEV;
	}
...
static bool nu1619_download_firmware_data(struct oplus_nu1619 *chip, char area)
{
	...
	msleep(10);
	if (area == BOOT_AREA) {
		ret = nu1619_wirte_firmware_data(chip, 0, chip->fw_boot_lenth);
		if (ret < 0) {
			return false;
		}
	}
	if (area == RX_AREA) {
		ret = nu1619_wirte_firmware_data(chip, 256, chip->fw_rx_lenth);
		if (ret < 0) {
			return false;
		}
	}
	if (area == TX_AREA) {
		ret = nu1619_wirte_firmware_data(chip, 4864, chip->fw_tx_lenth);
		if (ret < 0) {
			return false;
		}
	}
...
static bool nu1619_check_firmware_version(struct oplus_nu1619 *chip)
{
	...
	pr_err("<FW UPDATE> boot (0xff&(~fw_boot_data[fw_boot_lenth)-5]))=0x%x\n",
		(0xff & (~chip->fw_boot_data[chip->fw_boot_lenth - 5])));
	pr_err("<FW UPDATE> rx (0xff&(~fw_rx_data[fw_rx_lenth-5]))=0x%x\n",
		(0xff & (~chip->fw_rx_data[chip->fw_rx_lenth - 5])));
	pr_err("<FW UPDATE> tx (0xff&(~fw_tx_data[fw_tx_lenth-5]))=0x%x\n",
		(0xff & (~chip->fw_tx_data[chip->fw_tx_lenth - 5])));

	chip->fw_rx_version = rx_version;
	chip->fw_tx_version = tx_version;
	if ((boot_version == (0xff & (~chip->fw_boot_data[chip->fw_boot_lenth - 5]))) && (boot_checksum == 0x66)) {
		chip->boot_no_need_update = true;
		pr_err("<FW UPDATE> boot_no_need_update=true\n");
	}
	if ((rx_version == (0xff & (~chip->fw_rx_data[chip->fw_rx_lenth - 5]))) && (rx_checksum == 0x66)) {
		chip->rx_no_need_update = true;
		pr_err("<FW UPDATE> rx_no_need_update=true\n");
	}
	if ((tx_version == (0xff & (~chip->fw_tx_data[chip->fw_tx_lenth - 5]))) && (tx_checksum == 0x66)) {
...
CCC:
	...
	pr_err("<FW UPDATE> error_count=%d, pass_count=%d, chip->fw_rx_lenth=%ld\n",
...
static void nu1619_event_process(struct oplus_nu1619 *chip)
{
	...
	if (chip->rx_connected == true) {
		...
		if (temp[0] & NU1619_RX_ERR_OVP)
			pr_err("rx OVP happen!\n");
		if (temp[0] & NU1619_RX_ERR_OTP)
